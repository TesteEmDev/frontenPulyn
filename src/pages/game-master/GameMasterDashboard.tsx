...
interface MonsterStatusMonster {
  teamId: string;
  teamName: string;
  teamColor: string;
  monsterHp: number;
...
interface MonsterStatus {
  ...
  monsters?: MonsterStatusMonster[];
  progress?: MonsterStatusMonster[];
}
...
export default function GameMasterDashboard() {
  ...
  const handleGameEvent = useCallback((event: GameEvent) => {
    ...
    if (event.type === 'GAME_SELECTED') {
    ...
    } else if (event.type === 'MONSTER_PROGRESS' || event.type === 'MONSTER_SPECIAL_ATTACK' || event.type === 'MONSTER_TEAM_DEFEATED' || event.type === 'MONSTER_DEFEATED') {
      setTreasureStatus({ active: false, gameType: 'none' });
      ...
      setLastGameEvent({
        label: event.type === 'MONSTER_DEFEATED' ? 'Todos os monstros derrotados' : event.type === 'MONSTER_TEAM_DEFEATED' ? 'Monstro de equipe derrotado' : event.type === 'MONSTER_SPECIAL_ATTACK' ? 'Ataque especial' : 'Ataque confirmado',
                 ...
                 <div className="flex items-center justify-between gap-4">
                   <div>
                     ...
                     <p className="text-sm text-gray-400">
                       {treasureStatus.completed
                         ...
                         : treasureStatus.active
                           ? `Etapa ${treasureStatus.roundNumber || 1} — todos os participantes da equipe da vez precisam escanear o alvo.`
                     ...
                     {treasureStatus.completed && (
                       <Button
                         ...
                         onClick={async () => {
                           try {
                             setActionLoading('start');
                             await api.startGame(selectedGameId, activeGame?.name || 'Caça ao Tesouro', selectedEventId);
                             await loadTreasureStatus();
                             setGameRunning(true);
                           } catch (err) {
                             alert('Erro ao iniciar próxima rodada: ' + (err instanceof Error ? err.message : String(err)));
                           } finally {
                             setActionLoading(null);
                           }
                         }}
                    ...
                    <p className="text-xs text-accent mt-1">
                      {treasureStatus.teamsProgress?.length || treasureStatus.teamRaceTimes?.length || 0} equipes na expedição
                    </p>
                  ...
                  <div className="rounded-lg bg-dark-surface/50 p-3">
                    ...
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-3">
                      {treasureStatus.teamRaceTimes?.map(team => (
                        <div key={team.teamId} className="flex items-center justify-between gap-2 text-sm">
                          <span className="min-w-0 truncate" style={{ color: team.teamColor }}>{team.teamName}</span>
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      {monsterStatus.gameCompleted
                        ? 'Todos os monstros foram derrotados.'
                        : monsterStatus.active
                          ? 'Cada equipe enfrenta seu próprio monstro. Os ataques e HP são independentes.'
                          : 'Inicie o jogo para liberar os monstros.'}
                    </p>
                  </div>
                  <Badge variant={monsterStatus.gameCompleted ? 'success' : 'danger'}>
                    {(monsterStatus.monsters || monsterStatus.progress || []).filter(monster => monster.monsterDefeated).length}/{(monsterStatus.monsters || monsterStatus.progress || []).length} derrotados
                  </Badge>
                </div>
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  {(monsterStatus.monsters || monsterStatus.progress || []).map(monster => {
                    ...
                    return (
                      <div key={monster.teamId} className="rounded-lg bg-surface/50 p-3" style={{ border: `1px solid ${monster.teamColor || '#1E9BD7'}55` }}>
                        <div className="mb-2 flex items-center justify-between gap-2">
                          <span className="font-semibold" style={{ color: monster.teamColor || '#fff' }}>{monster.teamName}</span>
                          <Badge variant={defeated ? 'success' : 'danger'}>{defeated ? 'Derrotado' : `${hp}/${maxHp} HP`}</Badge>
                        </div>
                        <div className="h-3 rounded-full bg-dark-surface">
                          <div className="h-full rounded-full transition-all" style={{ width: `${maxHp ? Math.max(0, Math.min(100, (hp / maxHp) * 100)) : 0}%`, backgroundColor: monster.teamColor || '#ef4444' }} />
                        </div>
                        <p className="mt-2 text-xs text-gray-400">{monster.scanned}/{monster.total} participantes atacaram {monster.complete ? '• Ataque especial liberado' : ''}</p>
                      </div>
                    );
                  })}
                </div>
